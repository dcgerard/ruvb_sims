...
left_vals <- toupper(str_replace(str_extract(names(mse_mat)[-(1:5)], "^.+_"), "_", ""))
right_vals <- str_replace(str_extract(names(mse_mat)[-(1:5)], "_.+$"), "_", "")
name_vec <- c("Seed", "Pi0", "SampleSize", "NControls", "Poisthin",
              paste0(left_vals, right_vals))
name_vec[(length(name_vec) - 2):length(name_vec)] <- c("RUVBn", "RUVBnl", "RUVB")

stopifnot(length(name_vec) == ncol(mse_mat))

colnames(mse_mat) <- name_vec
colnames(cov_mat) <- name_vec
colnames(auc_mat) <- name_vec

cov_mat <- select(cov_mat, -Seed, -Poisthin)
auc_mat <- select(auc_mat, -Seed, -Poisthin)
mse_mat <- select(mse_mat, -Seed, -Poisthin)
...
longdat <- filter(longdat, Pi0 == 0.5)
pdf(file = "./Output/figures/coverage_5.pdf", height = 12, width = 15, family = "Times")
p <- ggplot(data = longdat, mapping = aes(y = Coverage, x = Method)) +
    geom_boxplot(outlier.size = 0.2, size = 0.2) +
...
dummy_dat <- cbind(expand.grid(nullpi_seq[nullpi_seq != 1], nsamp_seq, ncontrol_seq),
                   apply(med_mat, 1, max))
colnames(dummy_dat) <- c("Pi0", "SampleSize", "NControls", "max_med")

longdat <- gather(data = auc_mat, key = "Method", value = "AUC", -(1:3)) %>%
  filter(Pi0 != 1)
pdf(file = "./Output/figures/auc.pdf", height = 12, width = 15, family = "Times")
p <- ggplot(data = longdat, mapping = aes(x = Method, y = AUC)) +
    geom_boxplot(outlier.size = 0.2, size = 0.2) +
    facet_grid(Pi0 + NControls ~ SampleSize) +
...
## Diff AUC Plot ------------------------------------------------------------
diff_mat <- cbind(auc_mat[, 1:3], auc_mat[, -c(1:3)] - auc_mat$RUVBnl)
diff_mat <- select(diff_mat, -RUVBnl)

longdat <- gather(data = diff_mat, key = "Method", value = "AUC", -(1:3)) %>%
  filter(Pi0 != 1)
pdf(file = "./Output/figures/diff.pdf", height = 12, width = 15, family = "Times")
...
longdat <- filter(longdat, Pi0 == 0.5)
pdf(file = "./Output/figures/diff_5.pdf", height = 6, width = 15, family = "Times")
p <- ggplot(data = longdat, mapping = aes(y = AUC, x = Method)) +
    geom_boxplot(outlier.size = 0.2, size = 0.2) +
    facet_grid(NControls ~ SampleSize) +
    geom_hline(yintercept = 0, lty = 2) +
